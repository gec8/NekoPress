...
- 超过 6MB 的音视频使用 Supabase TUS 分块断点续传，支持进度、重试、暂停和恢复
- 媒体库图片使用长期缓存的响应式缩略预览，音视频按需加载并自动补全尺寸和时长
- 前台缺少文章封面或动态配图时，按内容标识生成稳定的在线随机图片，并保留本地加载失败兜底
- 媒体库优先读取媒体索引，首屏仅渲染 18 个项目并支持继续加载，避免大量文件拖慢后台
- 媒体库提供容量、使用状态统计，以及未使用媒体筛选和大文件排序，清理时继续执行引用保护
- 媒体一致性检查仅在手动触发时连接 Storage，可识别并安全修复缺失索引、文件丢失和中断上传状态
- 媒体上传、删除和索引修复会写入后台操作日志，记录操作者、文件和必要的结果摘要
- JSON 备份格式 v2 包含媒体资产清单、存储路径和内容引用关系；Storage 文件本体保持独立备份
- 系统页支持先验证预览、再确认合并恢复 v1/v2 JSON 备份，不会清空现有数据库
...
已有数据库升级时，按文件名顺序执行 `supabase/migrations/` 中尚未应用的迁移；其中包括回收站、版本历史、后台操作日志，以及 `202609070001_media_catalog.sql` 媒体资产与引用关系迁移。
