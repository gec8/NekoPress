...
export default async function AdminLayout({children}:{children:React.ReactNode}){
  ...
  if("error" in auth) return <main className="site-width pt-10"><div className="panel mx-auto max-w-xl p-7"><p className="text-xs font-black uppercase tracking-[.2em] text-red-500">后台不可用</p><h1 className="mt-2 text-3xl font-black">无法进入管理后台</h1><p className="mt-4 text-sm leading-7 text-zinc-500">{auth.error}</p><p className="mt-3 text-sm leading-7 text-zinc-500">{auth.kind==="network"?"登录服务当前不可达，请检查网络后重试。":auth.kind==="database"?"登录有效，但数据库暂时不可用，请稍后重试。":auth.kind==="permission"?"请确认当前账号在 profiles 表中拥有 admin 或 editor 角色。":"请检查 Supabase 地址和服务端密钥配置。"}</p></div></main>;
