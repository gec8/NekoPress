...
export async function GET() {
  ...
  const [database,storage,migration]=await Promise.all([
    timedCheck(async()=>auth.admin.from("site_settings").select("id").limit(1),"数据库连接正常","数据库读取异常"),
    timedCheck(async()=>{const {data,error}=await auth.admin.storage.getBucket(bucket);return {error:error??(!data?{message:`找不到 ${bucket} 存储桶`}:null)}},"媒体存储正常","媒体存储异常"),
    timedCheck(async()=>auth.admin.from("article_versions").select("id").limit(1),"回收站迁移已启用","尚未执行回收站迁移"),
  ]);
  const checks={authentication:{ok:true,latencyMs:0,message:"登录与后台权限正常"},database,storage,migration};
