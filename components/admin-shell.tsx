...
const titles:Record<string,string>={admin:"数据概览",articles:"文章管理",moments:"动态管理",comments:"评论审核",taxonomy:"分类与标签",carousel:"首页轮播",media:"媒体库",settings:"网站设置",new:"新建内容"};
...
export function AdminShell({ children,email,role }:{ children:React.ReactNode;email:string;role:"admin"|"editor" }) {
  ...
  async function logout(){await fetch("/api/auth/logout",{method:"POST"});window.location.replace("/auth/login")}
  ...
  return <div className="admin-system min-h-screen bg-[#f6f7f9] dark:bg-[#0b0d12]">
