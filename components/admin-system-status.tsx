...
const labels:Record<string,{label:string;description:string}>={authentication:{label:"登录权限",description:"确认当前会话和后台角色有效"},database:{label:"数据库",description:"确认内容与设置可以正常读取"},storage:{label:"媒体存储",description:"仅在手动完整检测时连接 Storage"},migration:{label:"内容数据迁移",description:"确认回收站和历史版本已启用"},mediaCatalog:{label:"媒体数据迁移",description:"确认媒体资产和使用关系可以正常追踪"}};
...
export function AdminSystemStatus(){
  ...
  return <div className="space-y-6">
    ...
    <section className="panel p-5 sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-start"><div className="flex flex-1 items-start gap-3"><DatabaseBackup className="mt-0.5 text-blue-500" size={22}/><div><h3 className="font-black">内容与媒体清单备份</h3><p className="mt-1 text-sm text-zinc-500">导出文章、动态、分类、轮播、设置、媒体元数据和引用关系，不包含账号、评论、密钥或媒体文件本体。</p></div></div><a href="/api/admin/export" className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-black text-white"><Download size={15}/>导出 JSON 备份</a></div><div className="mt-5 grid gap-3 md:grid-cols-3"><BackupItem title="内容数据" text="重大修改前下载 JSON 备份，并保存到网站服务器之外。"/><BackupItem title="媒体文件" text="备份包含媒体路径和引用关系；文件本体仍需定期复制 Storage 的 media 存储桶。"/><BackupItem title="完整数据库" text="同时在 Supabase 开启自动备份，并定期进行恢复演练。"/></div><div className="mt-4 flex items-start gap-2 rounded-xl bg-blue-500/10 p-3 text-xs leading-5 text-blue-700 dark:text-blue-300"><HardDrive size={16} className="mt-0.5 shrink-0"/>备份文件可能包含未发布内容，请妥善保管，不要提交到 GitHub。</div></section>
    <AdminBackupRestore/>
  </div>
}
