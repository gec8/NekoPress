...
import { AlertTriangle, CheckCircle2, DatabaseBackup, Download, HardDrive, RefreshCw, ShieldCheck } from "lucide-react";
...
const labels:Record<string,{label:string;description:string}>={authentication:{label:"登录权限",description:"确认当前会话和后台角色有效"},database:{label:"数据库",description:"确认内容与设置可以正常读取"},storage:{label:"媒体存储",description:"确认图片存储桶可访问"},migration:{label:"数据迁移",description:"确认回收站和历史版本已启用"}};
...
export function AdminSystemStatus(){
  ...
  const check=useCallback(async()=>{
    ...
    try{const response=await fetch("/api/admin/health",{cache:"no-store"});const payload=await response.json() as Payload;if(!response.ok&&!payload.checks)throw new Error(payload.message||"系统检测失败");setData(payload)}
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 sm:grid-cols-2">{Object.entries(data.checks??{}).map(([key,item])=><section className="panel p-5" key={key}><div className="flex items-start gap-3">{item.ok?<CheckCircle2 className="mt-0.5 shrink-0 text-emerald-500" size={20}/>:<AlertTriangle className="mt-0.5 shrink-0 text-amber-500" size={20}/>}<div><h3 className="font-black">{labels[key]?.label??key}</h3><p className="mt-1 text-xs text-zinc-400">{labels[key]?.description}</p><p className={`mt-3 text-sm font-bold ${item.ok?"text-emerald-600":"text-amber-600"}`}>{item.message}</p><p className="mt-1 text-xs text-zinc-400">耗时 {item.latencyMs}ms</p></div></div></section>)}</div></>}
